...
fn main() {
    let root = PathBuf::from(
        std::env::args_os()
            .nth(1)
            .expect("usage: verify_native_acceptance <run-directory>"),
    )
    .canonicalize()
    ...
    let original = ProjectStore::default()
        .load(&original_path)
        .expect("load original project");
    let relinked = ProjectStore::default()
        .load(&relinked_path)
        .expect("load relinked project");
    ...
    expected
        .set_source_uri(
            ...
            relinked.sources()[0].uri().clone(),
        )
        .unwrap();
    assert_eq!(
        expected, relinked,
        "Relink/Save As may change location only"
    );
    assert!(
        !root.join("voice.wav").exists(),
        "the original fixture must have been moved"
    );
    assert!(root.join("relocated.wav").is_file());

    let service = AppService::default();
    let opened = service
        .open_project(relinked_path.clone(), 0, false)
        .expect("fresh application Open");
    let project = opened.project.unwrap();
    let track_id = &project.tracks[0].id;
    let job = service
        .analyze_track(&project.id, opened.generation, track_id)
        .unwrap();
