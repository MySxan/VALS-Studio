...
- 本机通过 70 项核心测试、20 项前端测试、2 项 Tauri IPC 闭环、4 项 fixture 契约测试、工具脚本测试、五个示例及 fmt/Clippy/TypeScript/Vite/原生构建；已确认 fixture 字节与调用环境不变。

已知限制：无音频拷贝/自动扫描；PCM 上限 64 MiB，无磁盘 artifact 缓存，轨道持有 artifact 不计入 runtime 预算。真实原生对话框/关闭流程、Unix symlink 测试与远程 CI 尚未实机验收。隔离工具链布局仍是工作区约定，System 模式与跨平台 CI 尚未完整实机验证。完整 user override 编辑、playback、notes、lyrics、alignment、undo/redo、pitch、autosave 尚未实现。
