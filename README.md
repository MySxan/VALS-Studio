...
Tauri/React 支持 New/Open/Save/Save As/Close、WAV 导入、轨道选择、Canvas 波形与 CPAL 播放；音乐实体编辑尚未实现。
...
- `vocal-app::PlaybackEngine`：provider-independent 播放端口与 generation/track/source-scoped session；独立解码缓冲不进入工程或分析缓存。
- `apps/desktop`：Tauri 2 薄适配层、CPAL 0.18.2 原采样率输出、React/TypeScript transport、Zustand、Zod、Canvas 波形/游标与 provenance Inspector。
...
[分析切片](docs/architecture/phase-0-analysis-slice.md)、[视口切片](docs/architecture/phase-0-viewport-slice.md)、[后台会话切片](docs/architecture/phase-0-app-slice.md)与[播放切片](docs/architecture/phase-0-playback-slice.md)。
...
运行后 New 或 Open 工程，再 Import WAV；Save/Save As 保存 `.vocalproj`。Close/Open 后选中轨道会核验源并显示波形，源缺失或变化明确显示状态。支持取消、缩放、平移、从当前视口起点播放/暂停/seek/停止和分析依据查看。选择“重新关联音频”可恢复移动后的同内容 WAV，成功后保存工程；不同内容会被拒绝。工程内音频保存相对关联，目录整体移动后可恢复；Save As 重算路径但不复制音频。
...
播放变更本地通过：73 项核心测试、21 项前端测试及 Windows target 的 Tauri/CPAL check + Clippy；本轮原生 tests/build 待远端 CI。此前基线的 GitHub Actions 已通过 Ubuntu/macOS Core 与 Windows All/System。真实扬声器输出、原生对话框和窗口关闭仍待交互式 Windows 验收。
